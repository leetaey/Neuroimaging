...
전체과정에서 이 단계가 가장 중요하다. 제대로 skull stripping을 하지 않으면 엉터리 데이터로 분석하는 셈이 된다. skull stripping에서의 가장 큰 원칙 중 하나는 brain을 손상시키는 것 보다는 적게 벗겨내는 것이 더 낳다 라는 것이다. 따라서 다소간에 시간이 걸리고 수고스럽더라도 -f 0.1 부터 0.05 단위로 올리면서 반복해서 이미지를 처리해주고 잘 처리되는 것은 제외하고 남은것을 처리하는 방식으로 진행하는 것이 좋다. 
